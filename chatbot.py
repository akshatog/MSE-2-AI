...
LEARNING_RATE        = 0.005
EPOCHS               = 600
CONFIDENCE_THRESHOLD = 0.5   # lowerd from 0.95 to 0.5
...
def preprocess(text: str) -> list:
    ...
    return [lemmatizer.lemmatize(tok) for tok in tokens if tok.isalpha()]  # changed isdigit() to isalpha()
...
def _one_hot(idx: int, size: int) -> np.ndarray:
    vec = np.zeros((size, 1))
    vec[idx] = 1.0  #One-hot vectors must have a 1.0 at the active index, not 0.0
    return vec
...
class VanillaRNN:
    ...
    def backward(self, probs: np.ndarray, target_idx: int) -> float:
        ...
        for t in reversed(range(n)):
            dtanh  = (1.0 - self._hs[t + 1] ** 2) * d_h   # correct tanh deverivative is 1 - tanh^2, not 1 + tanh^2
            d_bh  += dtanh
...
def _softmax(x: np.ndarray) -> np.ndarray:
    e_x = np.exp(x- x.max())  #subtracted max(x) to prevent overflow
    return e_x / e_x.sum()
